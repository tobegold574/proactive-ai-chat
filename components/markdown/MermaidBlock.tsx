...
export function MermaidBlock({ chart }: { chart: string }) {
  ...
  useEffect(() => {
    ...
    const cached = cacheRef.current.get(normalizedChart)
    if (cached) {
      setSvg(cached)
      setError("")
      return
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        cacheRef.current.set(normalizedChart, result.svg)
        setSvg(result.svg)
      } catch (e) {
        if (cancelled) return
  ...
  return (
    <div className="my-2 relative rounded-lg bg-[#fdf6e3]">
      ...
      {mode === "source" ? (
        ...
        <div
          className="mermaid-svg overflow-auto p-3 pt-10"
          dangerouslySetInnerHTML={{ __html: svg }}
        />
      ) : (
        <div className="rounded-lg bg-[#eee8d5] p-3 pt-10 text-sm text-[#657b83]">
          Rendering…
        </div>
      )}
    </div>
